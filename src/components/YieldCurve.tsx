...
const YieldCurve = forwardRef(function ReffedYieldCurve(
  {
    allLines,
    chartData,
    containerWidth,
  ...
  }, [blurbsData, chartData, drawOneLine, height, lightColor, ref, setAllLines, width]);
  ...
  useEffect(() => {
    if (allLines?.length > 0 && currentIndex >= 0) {
      ...
      const dot = svg.append("g").attr("display", "none").classed("dots", true);
      dot.append("circle").attr("r", 2.5);
      dot.append("text").attr("text-anchor", "middle").attr("y", -8);
    }
  }, [allBlurbIndex, allLines, blurbsData, currentIndex, drawOneLine, height, isSimpleLayout, lightColor, mainColor, ref]);
